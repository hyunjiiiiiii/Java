public class BinaryTree<T> {
    ...
    private int heightHelper(TreeNode<T> l) {
        if (l == null) {
            return 0;
        }else if (l.left == null && l.right == null) {
            return 1;
        } else {
            int height = 1 + Math.max(heightHelper(l.left), heightHelper(l.right));
            return height;
        }
    }
    ...
    private boolean isCompletelyBalancedHelper(TreeNode<T> t) {
        if (heightHelper(t) == 0 || heightHelper(t) == 1) {
            return true;
        }
        return isCompletelyBalancedHelper(t.left) && isCompletelyBalancedHelper(t.right) && heightHelper(t.left) == heightHelper(root.right);
    ...
    private TreeNode<T> fibTreeHelper(int N) {
        if (N == 0) {
        ...
        return new TreeNode(new Integer(((Integer) left.item).intValue() + ((Integer) right.item).intValue()), left, right);
    }
