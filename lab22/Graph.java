...
public class Graph implements Iterable<Integer> {
    ...
    public void addEdge(int v1, int v2, int weight) {
        // TODO: YOUR CODE HERE
        if (!isAdjacent(v1, v2)) {
            LinkedList<Edge> v1Neighbor = adjLists[v1];
            v1Neighbor.add(new Edge(v1, v2, weight));
        } else {
            LinkedList<Edge> v1Neighbor = adjLists[v1];
            for (Edge edge : v1Neighbor) {
                if (edge.to()  == v2) {
                    edge.weight =weight;
                }
            }
    ...
    public boolean pathExists(int start, int stop) {
        // TODO: YOUR CODE HERE
        for (Integer visited : visited(start)) {
            if (visited == stop) {
                return true;
            }
        }
    ...
    private ArrayList<Integer> visited(int start) {
    ...
    public List<Integer> path(int start, int stop) {
        ...
        String pathString = pathHelper(start, stop, "");
        List<Integer> foundPath = new ArrayList<>();
        if (pathExists(start, stop) && pathString != "") {
            for (int i = 0; i < pathString.length(); i++) {
                Character digitCurrent = pathString.charAt(i);
                foundPath.add(Character.getNumericValue(digitCurrent));
            }
        }
        return foundPath;
    }

    private String pathHelper(int start, int stop, String path) {
        String vCurrent = Integer.toString(start);
        if (path.contains(vCurrent)) {
            return "";
        } else if (start == stop) {
            return path + vCurrent;
        } else {
            path += vCurrent;
        }
        ...
        for (int n : neighbors) {
            String s = "";
            if (!path.contains(Integer.toString(n))) {
                s = pathHelper(n, stop, path);
            }
            if (s.contains(Integer.toString(stop))) {
                path = s;
                break;
            }
        }
    ...
    private class TopologicalIterator implements Iterator<Integer> {
        ...
        private Integer[] inDegreeCurrent;
        private HashSet<Integer> visited;

        TopologicalIterator() {
            fringe = new Stack<Integer>();
            // TODO: YOUR CODE HERE
            inDegreeCurrent = new Integer[adjLists.length];
            visited = new HashSet<Integer>();
            int degree;

            for (int i = 0; i <adjLists.length; i++) {
                degree = inDegree(i);
                inDegreeCurrent[i] = degree;
                if (degree == 0) {
                    fringe.push(i);
                }
            }
        ...
        public Integer next() {
            ...
            for (Edge edge : adjLists[vCurrent]) {
                inDegreeCurrent[edge.to()]--;
            }
            visited.add(vCurrent);
            for (int i = 0; i <adjLists.length; i++) {
                if (!visited.contains(i) && !fringe.contains(i) && inDegreeCurrent[i] == 0) {
                    fringe.push(i);
                }
            }
            return vCurrent;
