...
public class Graph {
    ...
    public Graph prims(int start) {
        // TODO: YOUR CODE HERE
        Graph T = new Graph();
        TreeMap<Integer, Edge> dist = new TreeMap<>();
        PriorityQueue<Integer> fringe = new PriorityQueue<>(Comparator.comparingInt(v -> dist.get(v).getWeight()));
        TreeSet<Integer> all = getAllVertices();

        fringe.add(start);
        dist.put(start, new Edge(0, 0, 0));
                ...
                for (Edge edge : getEdges(v)) {
                    ...
                    if (all.contains(dest)) {
                        if (!dist.containsKey(dest)) {
                            dist.put(dest, edge);
                            fringe.add(dest);
                        } else if (edge.getWeight() < dist.get(dest).getWeight()) {
                            dist.put(dest, edge);
                            fringe.add(dest);
                        }
                    }
    ...
    public Graph kruskals() {
        ...
        for (Edge edge : getAllEdges()) {
            if (!u.connected(edge.getSource(), edge.getDest())) {
                u.union(edge.getSource(), edge.getDest());
                T.addEdge(edge.getSource(), edge.getDest(), edge.getWeight());
            }
        }
        return T;
    }
